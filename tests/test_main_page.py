...
class TestMainPage:
    ...
    def test_pay_taxes_from_main_page(self, authorized_user):
        ...
        Шаги:
           ...
           3) Кликнуть на " Проверить налоги"
           4) Выбрать первый счет в списке и кликнуть "Оплатить"
           5) Нажать "Дальше"
           ОР: Появилось предупреждение " В демо-версии переводы не разрешены"
        """
        authorized_user.open_main_page()
    ...
    def test_add_and_delete_private_event(self, authorized_user):
        """
           Шаги:
               1) Перейти на главную страницу
               2) Кликаем снизу-справа на "Добавить личное событие"
               3) Ввести произвольное название
               4) Ввести описание ( до 1000 символов)
               5) Нажать "Сохранить"
               6) На странице "Календарь событий" выбрать созанное событие
               7) Нажать "Удалить"
               ОР: Событие удалено из календаря
        """
        authorized_user.open_main_page()
    ...
    def test_add_and_delete_private_event_with_empty_fields(self, authorized_user):
        """
           Шаги:
               ...
               2) Кликаем снизу-справа на "Добавить личное событие"
               3) Нажать "Сохранить"
               ОР : Появилось предупреждение "Обязательное поле "
               под полями "Название" и "Дата"
        """
        authorized_user.open_main_page()
        authorized_user.main_page.click_private_event_button()
